import Departments from "../administrative-structure/department/departments";
      ...
      success: function(data) {
        if (data.psa) {
          for (i = 0; i < data.psa["source"].length; ++i) {
            pastExams +=
              '<div class="col-lg-2 col-md-3 col-sm-4 col-xs-6 catssys-test">' +
              '<a href="' +
              data.psa_dir +
              "/" +
              data.psa["source"][i] +
              '" target="_blank">' +
              '<i class="fa fa-file-text-o"></i><h4>' +
              data.psa["number"][i] +
              "º" +
              " PSA " +
              data.psa["year"][i];
        ...
        if (data.recruitments !== null) {
          ...
          if (studentData.content !== null) {
            ...
            studentWrapper
              ...
              .prepend(
                "<b>Inscrições abertas de <br>" +
                  moment(
                    studentData.content.recruitmentBeginDate.date,
                    "YYYY-MM-DD"
                  ).format("LL") +
                  " a " +
                  moment(
                    studentData.content.recruitmentEndDate.date,
                    "YYYY-MM-DD"
                  ).format("LL") +
                  "</b>"
              );
            ...
            studentWrapper
              ...
              .attr(
                "href",
                "/recruitment/recruitment/public-notice/" +
                  studentData.content.recruitmentId
              )
              .html(
                studentData.content.recruitmentNumber +
                  "º " +
                  "Processo Seletivo de Alunos " +
                  studentData.content.recruitmentYear +
                  " (<i class='fa fa-file-pdf-o'></i>)"
              );
            studentWrapper
              .find(".recruitment-form-wrapper-closed")
              .html(
                "O formulário de inscrição estará disponível no período de " +
                  moment(
                    studentData.content.recruitmentBeginDate.date,
                    "YYYY-MM-DD"
                  ).format("L") +
            ...
            if (studentData.offset) {
              studentWrapper.find(".recruitment-form-wrapper-closed").show();
            } else {
              if (studentData.showSubscriptionLink) {
                studentWrapper.find(".recruitment-form-wrapper-opened").show();
              }

              studentWrapper.find(".recruitment-form-wrapper-candidate").show();
            }
          ...
          if (volunteerData.content !== null) {
            ...
            volunteerWrapper
              ...
              .prepend(
                "<b>Inscrições abertas de <br>" +
                  moment(
                    volunteerData.content.recruitmentBeginDate.date,
                    "YYYY-MM-DD"
                  ).format("LL") +
                  " a " +
                  moment(
                    volunteerData.content.recruitmentEndDate.date,
                    "YYYY-MM-DD"
                  ).format("LL") +
                  "</b>"
              );
            ...
            volunteerWrapper
              ...
              .attr(
                "href",
                "/recruitment/recruitment/public-notice/" +
                  volunteerData.content.recruitmentId
              )
              .html(
                volunteerData.content.recruitmentNumber +
                  "º " +
                  "Processo Seletivo de Voluntários " +
                  volunteerData.content.recruitmentYear +
                  " (<i class='fa fa-file-pdf-o'></i>)"
              );
            volunteerWrapper
              .find(".recruitment-form-wrapper-closed")
            ...
            if (volunteerData.offset) {
              volunteerWrapper.find(".recruitment-form-wrapper-closed").show();
            } else {
              if (volunteerData.showSubscriptionLink) {
                volunteerWrapper
                  .find(".recruitment-form-wrapper-opened")
                  .show();
              }

              volunteerWrapper
                .find(".recruitment-form-wrapper-candidate")
                .show();
            }
          ...
          switch (type) {
            case 1:
              studentWrapper.addClass(
                "col-lg-12 col-md-12 col-sm-12 col-xs-12"
              );
              studentWrapper.show();
              break;
            case 2:
              volunteerWrapper.addClass(
                "col-lg-12 col-md-12 col-sm-12 col-xs-12"
              );
            ...
            case 3:
              studentWrapper.addClass(
                "col-lg-6 col-md-6 col-md-push-6 col-sm-12 col-xs-12"
              );
              volunteerWrapper.addClass(
                "col-lg-6 col-md-6 col-md-pull-6 col-sm-12 col-xs-12"
              );
              studentWrapper.show();
              volunteerWrapper.show();
              break;
