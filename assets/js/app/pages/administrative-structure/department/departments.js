module.exports = (function() {
  ...
  getAdministrativeHierarchy = function() {
    ...
    $.ajax({
      url:
        "/administrative-structure/department/getDepartments" +
        (params.length === 1 ? "/" + params[0] : ""),
      success: function(data) {
        var departments = $(
          "<div id='department-" +
            (params.length !== 0 ? params[0] : "root") +
            "' style='display: none;'>"
        );
        ...
        if (params.length !== 0) {
          departments.append(
            "<br><h3>" +
              "<i class='" +
              departmentObjArray[params[0]].departmentIcon +
              "'></i> " +
              departmentObjArray[params[0]].departmentName +
              "</h3><hr>" +
              "<p class='text-justify'>" +
              departmentObjArray[params[0]].departmentDescription.replace(
                /\n/g,
                "<br>"
              ) +
              "</p><br>"
          ...
          departments
            ...
            .append(
              "<li data-active='" +
                ...
                (typeof adminStructure.data("catssys-toolbar") !== "undefined"
                  ? "cats-row"
                  : "") +
                (results[i].isActive === false
                  ? " catssys-admin-structure-disabled"
                  : "") +
                "'>" +
                "<div href='#department-" +
                results[i].departmentId +
                "' data-toggle='tab'>" +
                "<i class='" +
                results[i].departmentIcon +
                "'></i>" +
                "<h4><strong>" +
                results[i].departmentName +
                "</strong></h4>" +
                "<p class='text-justify'>" +
                results[i].departmentDescription +
                "</p></div></li>"
            );
        }
        ...
        if (params.length !== 0) {
          ...
          adminStructure.find(".container").append(departments);
        }
  ...
  return {
    ...
    getCallbackOf: function(element) {
      return {
        exec: function(data) {
          adminStructure
            .find("#department-identity-" + data.departmentId)
            .remove();
